...
- [x] **`S`** `docker-compose.yml`: блок `volumes:` стоит с отступом внутри `services:` — вынести на верхний уровень, иначе compose невалиден.
...
- [x] **`L`** Реструктуризатор: планировщик (apscheduler или отдельный контейнер), кластеризация по эмбеддингам, предложения в боте с подтверждением, перемещение заметок. Учесть каскад: путь меняется в SQL, в Chroma metadata и в wikilinks других заметок.
